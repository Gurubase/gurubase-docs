...
3. Slack will send events to  `/api/slack/events/` endpoint of your Gurubase frontend. If the frontend URL is not public, you need to open it to public. You can use [ngrok](https://ngrok.com/) to do this:
   - If you haven't changed the default ports of Gurubase, port 8029 is for the frontend UI. You need to use this to make it accessible to the public.
   - Make frontend URL public with `ngrok http http://localhost:8029`
...
5. Set your public url along with the path `/api/slack/events/` as "Request URL", this is the url that Slack will send the events to. Be careful with the trailing slash at the end of the url.
    - `${your_public_frontend_url}/api/slack/events/`

<Note type="warning">
If you change the frontend URL, you need to update the "Request URL" with the new URL.
</Note>
